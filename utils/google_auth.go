...
func GenGoogleSecret() (string, error) {
	return googleAuth.GetSecret()
}

func VerifyGoogleCode(secret, code string) (bool, error) {
	return googleAuth.VerifyCode(secret, code)
}
...
// 获取秘钥
func (this *GoogleAuth) GetSecret() (string, error) {
	var buf bytes.Buffer
	err := binary.Write(&buf, binary.BigEndian, this.un())
	return strings.ToUpper(this.base32encode(this.hmacSha1(buf.Bytes(), nil))), err
}
...
func initAuth(user string) (secret, code string) {
	// 秘钥
	secret, _ = NewGoogleAuth().GetSecret()
